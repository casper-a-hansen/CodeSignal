        ...
        string[] cellsJoining(string[] table, int[][] coords) {
            ...
            if (bottom - top > 0) {
                for(var r = top + 1; r < bottom + 1; r++) {
                    var blank = (left == 0 ? '|' : '+') + new string(' ', columns[right + 1] - columns[left] - 1) + (right + 1 == columns.Length - 1 ? '|' : '+');
                    table[rows[r]] = table[rows[r]].Substring(0, columns[left]) + blank + table[rows[r]].Substring(columns[right + 1] + 1);
                }
            }
            return table;
        }
