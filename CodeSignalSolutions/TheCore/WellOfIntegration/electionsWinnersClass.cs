/*
    ...
    Description:
        ...
        The second candidate can win if all the remaining candidates vote for him (3 + 3
        = 6 > 5).
        ...
        his opponents, he will still be the winner (the votes array will thus be [3, 4,
        5, 3]).
...
namespace CodeSignalSolutions.TheCore.WellOfIntegration
{
    class electionsWinnersClass
    {
        int electionsWinners(int[] votes, int k) {
            int winner = votes.Max();
            if (k == 0) {
                if (votes.Count(v => v == winner) == 1)
                {
                    return 1;
                }
                return 0;
            }
            var result = votes.Count(v => v + k > winner);
            return result;
        }
    }
}
