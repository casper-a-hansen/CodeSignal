/*
    ...
    Description:
        ...
        some numbers have a modified single digit in their binary representation. More
        specifically, in the given number n the kth bit from the right was initially set
        to 0, but its current value might be different. It's now up to you to write a
        ...
        The
        4th bit is 0 already (looks like the Mad Coder forgot to encrypt this number),
...
{
    class killKthBitClass
    {
        int killKthBit(int n, int k)
        {
          return n & ((1 << (k - 1)) ^ 0x7FFFFFFF);
        }
    }
}
