/*
    ...
    Description:
        ...
        You
        are given the initial configuration of the board and a list of commands which
        the snake follows one-by-one. The game ends if one of the following happens:
        the snake tries to eat its tail;
        the snake tries to move out of the board;
        it
        executes all the given commands.
        
        Output the board configuration after the game
        ends.
        ...
        and commands =
        "FRFFRFFRFLFF", the output should be
        ...
        A rectangular matrix of characters. It is guaranteed that it
        represents a correct game board configuration, i.e. there is exactly one snake.
        Direction of snake's head is depicted by one of the following characters ('^',
        '>', 'v', '<'). All of the other snake's body parts are depicted by '*'s (note,
        that if the snake has length 1 then there is no asterisks in its
        representation). All cells which are not occupied by the snake are depicted by
        '.'s.
        ...
        1 ≤
        gameBoard[0].length ≤ 10.
        
        [input] string commands
        
        A list of commands, where
        'F' means go one cell forward in the current direction, 'L' and 'R' mean change
        current direction 90 degrees left (counter-clockwise) or right (clockwise)
        correspondingly.
        ...
        If the
        snake dies, output its state before the losing move and replace each of the
        cells it occupied with Xs.
