/*
    ...
    Description:
        ...
        given array
        a = [a1, a2, ..., an]. He started with summing and did it easily, but decided to
        store the sum he found in some random position of the original array which was a
        bad idea. Now he needs to cope with the second task, sorting the original array
        ...
        1 + 3 + 6 + 2 = 12, which means that 1, 3, 6 and 2 are original elements of the
        array.
        ...
        Array of at least two integers. It is guaranteed that there is an index i such
        that shuffled[i] = shuffled[0] + ... + shuffled[i - 1] + shuffled[i + 1] + ... +
        shuffled[n].
...
{
    class shuffledArrayClass
    {
        int[] shuffledArray(int[] shuffled) {
            int sum = shuffled.Sum() / 2;
