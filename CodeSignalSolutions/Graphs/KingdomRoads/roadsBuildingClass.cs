...
{
    class roadsBuildingClass
    {
        int[][] roadsBuilding(int cities, int[][] roads) {
            var existing = roads.Select(r => r[0] < r[1] ? r[0] * 256 + r[1] : r[1] * 256 + r[0]).ToHashSet();
            return Enumerable
                .Range(0, cities - 1)
                .SelectMany(i => Enumerable
                    .Range(i + 1, cities - i - 1)
                    .Where(j => !existing.Contains(i * 256 + j))
                    .Select(j => new []{i,j})).ToArray();
        }
    }
}
