/*
    ...
    Description:
        ...
        His kingdom in the middle of a financial crisis, Byteasar was struggling to keep
        the economy out of a recession. Unfortunately there was not much he could do,
        and after much agonizing he came to the only solution: one of his cities had to
        be demolished, since keeping communication active between all the cities is way
        too expensive.
        It is not yet known if Byteasar chose a city to destroy after careful planning
        or picked one at random. As a person with a PhD in History and Nobel prize in
        Computer Science, you can solve this mystery! Archaeologists have recently found
        a manuscript with information about the roads between the cities, that is now
        stored in the roadRegister matrix. You want to try and remove each city one by
        one and compare the road registers obtained this way. Thus you'll be able to
        compare the obtained roads and determine whether the one picked by Byteasar was
        ...
        And here's how the cities are connected with one of the cities destroyed (cities
        0 - 3, respectively):
        Input/Output
        ...
        resulting array should contain the road register of the kingdom with the ith
        city removed.
*/
using System;
...
{
    class financialCrisisClass
    {
        bool[][][] financialCrisis(bool[][] roadRegister) {
            return Enumerable.Range(0, roadRegister.Length)
                .Select(removed => Enumerable.Range(0, roadRegister.Length)
                    .Where(to => to != removed)
                    .Select(to => Enumerable.Range(0, roadRegister.Length)
                        .Where(from => from != removed)
                        .Select(from => roadRegister[to][from])
                        .ToArray())
                    .ToArray())
                .ToArray();
        }
