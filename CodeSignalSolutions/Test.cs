...
namespace CodeSignalSolutions
{
    public static class Test
    {
        ...
        static int ReadInt(string value) => int.Parse(value.Trim());
        static string ReadString(string value)
        {
            var v = value.Trim();
            if (v[0] != '\"') throw new ArgumentException(nameof(value), "String value not starting with quotation mark");
            if (v[v.Length - 1] != '\"') throw new ArgumentException(nameof(value), "String value not ending with quotation mark");
            return v.Substring(1, v.Length - 2).Replace("\\\"", "\"");
        }
        static char ReadChar(string value) => ReadString(value)[0];
        static int[] ReadIntArray(string value)
        {
            var v = value.Trim();
            if (v[0] != '[') throw new ArgumentException(nameof(value), "Array value not starting with square paranthese");
            if (v[v.Length - 1] != ']') throw new ArgumentException(nameof(value), "Array value not ending with square paranthese");
            v = v.Substring(1, v.Length - 2);
            return v.Split(',').Select(s => ReadInt(s)).ToArray();
        }
        static string[] ReadStringArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return v.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(s => ReadString(s)).ToArray();
        }
        static char[] ReadCharArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return v.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(s => ReadChar(s)).ToArray();
        }
        static Regex regexIntArrays = new Regex(@"\s*(\[(?:\s*\d+\s*,?)*\])\s*,?", RegexOptions.Singleline);
        static int[][] ReadIntDArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return regexIntArrays.Matches(v).Cast<Match>().Select(s => ReadIntArray(s.Groups[1].Value)).ToArray();
        }
        static Regex regexStringArrays = new Regex(@"\s*(\[(?:\s*"".*?""\s*,?)*\])\s*,?", RegexOptions.Singleline);
        static string[][] ReadStringDArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return regexStringArrays.Matches(v).Cast<Match>().Select(s => ReadStringArray(s.Groups[1].Value)).ToArray();
        }
        static char[][] ReadCharDArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return regexStringArrays.Matches(v).Cast<Match>().Select(s => ReadCharArray(s.Groups[1].Value)).ToArray();
        }
        static Regex regexStringDArrays = new Regex(@"\s*(\[\s*(?:\[(?:\s*"".*?""\s*,?)*\])\s*,?\])\s*,?", RegexOptions.Singleline);
        static char[][][] ReadCharTArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return regexStringDArrays.Matches(v).Cast<Match>().Select(s => ReadCharDArray(s.Groups[1].Value)).ToArray();
        }
        static string[][][] ReadStringTArray(string value)
        {
            var v = value.Trim();
            Debug.Assert(v[0] == '[');
            Debug.Assert(v[v.Length - 1] == ']');
            v = v.Substring(1, v.Length - 2);
            return regexStringDArrays.Matches(v).Cast<Match>().Select(s => ReadStringDArray(s.Groups[1].Value)).ToArray();
        }
    }
}
