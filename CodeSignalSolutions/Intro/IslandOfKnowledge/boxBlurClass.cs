/*
    ...
    Description:
        ...
        7 + 1 + 1 + 1 + 1) = 15 / 9 = 1.66666 = 1. The border pixels are cropped from
        the final result.
        For
        image = [[7, 4, 0, 1],
        [5, 6, 2, 2],
        ...
        integers in the blurred output. To get the first value: (7 + 4 + 0 + 5 + 6 + 2 +
        6 + 10 + 7) = 47 / 9 = 5.2222 = 5. The other three integers are obtained the
...
namespace CodeSignalSolutions.Intro.IslandOfKnowledge
{
    class boxBlurClass
    {
        int[][] boxBlur(int[][] image) {
            ...
            {
                int sum = image[y - 1][x - 1] + image[y - 1][x] + image[y - 1][x + 1] +
                          image[y][x - 1] + image[y][x] + image[y][x + 1] +
                          image[y + 1][x - 1] + image[y + 1][x] + image[y + 1][x + 1];
                return sum / 9;
            }
            var result = Enumerable.Range(0, image.Length - 2).Select(i => new int[image[0].Length - 2]).ToArray();
            for(var y = 1; y < image.Length - 1; y++)
            {
                var row = image[y];
                for(var x = 1; x < row.Length - 1; x++)
                {
                    result[y - 1][x - 1] = Blur(x, y);
                }
            }
            return result;
        }
